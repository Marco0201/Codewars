...
def count_loop_iterations(arr):
    results = []  # empty list
    inc = 1  # incrimental multiplier
    for i in arr:  # i is individual tuple || ex. i = (4, True)
        # sum of each i || ex. (4, True) -> 4 + 1 -> 5 || side note True = 1 and False = 0
        num = sum(i)
        # as it loops append the function inside
        results.append(inc * (num + 1))
        inc *= num  # inc is multiplied by the current num and as it loops
    return results
